...
const emit = defineEmits(['deleteTodo', 'editTodo', 'updateStatus'])
</script>
    ...
    <div v-if="ifTodoExist" class="shadow-md rounded px-8 py-4 m-2">
        <ul>
            ...
            <li v-for="todo in todos" :key="todo.id"
                ...
                <div :class="{ 'm-2 w-40 text-center': true, 'line-through': todo.completed }">{{ todo.todo }}</div>
                <input type="checkbox" class="w-6 h-6 m-2 rounded" v-model="todo.completed"
                    @change="$emit('updateStatus', todo.id)">
                <div>
                    <button class="bg-purple-500 p-2 text-white rounded m-2"
                        @click="$emit('editTodo', todo.id)">Edit</button>
